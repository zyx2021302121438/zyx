...
def read_st_graph(args):
    ...
    edges = {}
    edges["positive_edges"] = [edge[0:2] for edge in dataset if edge[2] > 0]
    edges["negative_edges"] = [edge[0:2] for edge in dataset if edge[2] < 0]
    edges["ecount"] = len(dataset)
    edges["ncount"] = len(set([edge[0] for edge in dataset] + [edge[1] for edge in dataset]))
    edges["scount"] = len(set([edge[0] for edge in dataset]))
    edges["positive_weights"] = [edge[2] for edge in dataset if edge[2] > 0]
    edges["negative_weights"] = [edge[2] for edge in dataset if edge[2] < 0]
    return edges
...
# def train_test_split(edges, test_size):
#     nodes = [edge[0] for edge in edges] + [edge[1] for edge in edges]
#     counter = Counter(nodes)
#     high_degree_nodes = [item for item, count in counter.items() if count > 5]
#     high_degree_edges = [edge for edge in edges if edge[0] in high_degree_nodes and edge[1] in high_degree_nodes]
#     test_size = int(test_size * len(edges))
#     test_edges = random.sample(high_degree_edges, min(test_size, len(high_degree_edges)))
...
def train_test_split(edges, test_size):
    nodes = [edge[0] for edge in edges] + [edge[1] for edge in edges]
    counter = Counter(nodes)
    edges_sorted_by_degree = sorted(edges, key=lambda edge: counter[edge[0]] + counter[edge[1]], reverse=True)
    half_size = int(0.5 * len(edges))
    candidate_edges = edges_sorted_by_degree[:half_size]
    test_size = int(test_size * len(edges))
    test_edges = random.sample(candidate_edges, test_size)
    train_edges = [edge for edge in candidate_edges if edge not in test_edges]
    return train_edges, test_edges


def setup_features(args, positive_edges, negative_edges, node_count, positive_weights, negative_weights):
    """
    ...
    if args.spectral_features:
        X = create_spectral_features(args, positive_edges, negative_edges, node_count, positive_weights,
                                     negative_weights)
    else:
...
def create_spectral_features(args, positive_edges, negative_edges, node_count, positive_weights, negative_weights):
    """
    ...
    values = positive_weights + positive_weights + negative_weights + negative_weights
