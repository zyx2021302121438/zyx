...
from utils import calculate_auc, setup_features, train_test_split
...
class SignedGCNTrainer(object):
    ...
    """
    def __init__(self, args, edges, edges_st):
        """
    ...
    def setup_dataset(self):
        ...
        self.X = setup_features(self.args,
                                self.edges_st["positive_edges"],
                                self.edges_st["negative_edges"],
                                self.edges_st["ncount"],
                                self.edges_st["positive_weights"],
                                self.edges_st["negative_weights"])[:-self.edges_st["scount"]]
    ...
    def score_link_model(self, epoch):
        """
        Score the model on the test set edges in each epoch.
        :param epoch: Epoch number.
        """
        loss, self.train_z = self.model(self.positive_edges, self.negative_edges, self.y)
        test_y_edges = self.test_positive_edges + self.test_negative_edges
        test_y_num = len(test_y_edges)
        test_n_edges = []
        for i in range(self.edges["ncount"] - 1):
            for j in range(i, self.edges["ncount"]):
                if np.random.rand() < 0.001:
                    if [i, j] not in self.edges["positive_edges"] and [i, j] not in self.edges["negative_edges"]:
                        test_n_edges.append([i, j])
                if len(test_n_edges) > test_y_num:
                    break
            if len(test_n_edges) > test_y_num:
                break
        score_positive_edges = torch.from_numpy(np.array(test_y_edges, dtype=np.int64).T).type(torch.long).to(self.device)
        score_negative_edges = torch.from_numpy(np.array(test_n_edges, dtype=np.int64).T).type(torch.long).to(self.device)
        test_positive_z = torch.cat((self.train_z[score_positive_edges[0, :], :], self.train_z[score_positive_edges[1, :], :]), 1)
        test_negative_z = torch.cat((self.train_z[score_negative_edges[0, :], :], self.train_z[score_negative_edges[1, :], :]), 1)
        scores = torch.mm(torch.cat((test_positive_z, test_negative_z), 0), self.model.regression_weights.to(self.device)) + self.model.regression_bias.to(self.device)
        probability_scores = torch.exp(F.softmax(scores, dim=1))
        predictions = probability_scores[:, 0:2].sum(1) / probability_scores[:, :].sum(1)
        predictions = predictions.cpu().detach().numpy()
        targets = [0] * len(test_y_edges) + [1] * len(test_n_edges)
        auc, f1, pos_ratio = calculate_auc(targets, predictions, self.edges)
        print("link prediction:", auc, f1)
